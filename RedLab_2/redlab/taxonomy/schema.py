...
class NetworkRole(str, Enum):
    """Where a vector sits in a money-flow graph motif.

    RedLab_1 used graph-derived FEATURES (entity fan-in/fan-out counts) inside
    a tabular model as a cheap proxy for network structure. This field is what
    lets RedLab_2's generator instead render a vector as an actual graph motif
    - a shape the simulator builds explicitly - so the GNN detector has real
    topology to learn from, not just a scalar summary of it.
    """

    NONE = "none"                      # no meaningful multi-hop network shape
    ORIGINATOR = "originator"          # source of funds entering the network
    LAYERING_HOP = "layering_hop"      # intermediate pass-through node
    COLLECTION_POINT = "collection_point"  # sink where value is extracted
    FAN_OUT = "fan_out"                # one source funding many downstream nodes
    FAN_IN = "fan_in"                  # many sources converging on one node
...
class AttackVector(BaseModel):
    ...
    network_role: NetworkRole = Field(
        default=NetworkRole.NONE,
        description="Graph motif role. NONE for vectors with no meaningful "
        "multi-hop network shape; the world-simulator graph generator only "
        "renders explicit topology for non-NONE vectors.",
    )
